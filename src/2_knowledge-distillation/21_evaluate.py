...
class DistilledRobertaMultiTask(nn.Module):
...
# Load distillation model configurations and weights
model_dir = "./robert-distilled-model"
config = AutoConfig.from_pretrained("hfl/chinese-roberta-wwm-ext")
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

model = DistilledRobertaMultiTask(config).to(device)
...
plt.savefig('confusion_matrix_distilled.png', bbox_inches='tight')
plt.show()
print("Confusion matrix has been saved as confusion_matrix_distilled.png")
...
果
with open('evaluation_results_distilled.json', 'w', encoding='utf-8') as f:
    json.dump(results, f, indent=4, ensure_ascii=False, cls=NumpyEncoder)

print("\nEvaluation results have been saved as evaluation_results_distilled.json")